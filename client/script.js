...
function onQuestion() {
  ...
  if (questionTextbox) {
    ...
    fetch('/sendQuestion', {
      ...
      body: JSON.stringify({userhash: hash, question: questionValue })
    })
  } else {
      ...
      if (data.queries) {
        ...
        for (const query in queries) {
          if (queries.hasOwnProperty(query)) {
            ...
            cardText.textContent = queries[query];
...
function fetchdebate()
{
  ...
  .then(data => {
    if (data.debate) {
      ...
      if (data.debate === previousDebate)
      {
        return;
      }
      previousDebate = data.debate;
      ...
      console.error('No debate found for this user.');
    }
  })
